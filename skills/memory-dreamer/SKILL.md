...
- Light fixes are mechanical and unbounded each cycle: add missing reverse edges between addressable headings/nodes (skip the one-way types `bak:`, `up:`, `out:`, `in:`), remove duplicate edges, remove self-edges, remove edges pointing to ids that no longer exist, remove child-to-containing-heading edges that only express containment, upgrade obvious `rel:` edges to a more specific type when the precedent in the table's example column clearly fits (examples only, not a template; do not follow rigidly, stay flexible to context).
