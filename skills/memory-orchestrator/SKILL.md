...
- For updates, send a brief to the same curator subagent and **proceed without waiting** for its reply (fire-and-forget). Include enough context for the curator to place the memory well. Do not send an isolated memory brief; include enough surrounding context for the curator to understand its meaning, relevance, and relationship to existing memory before deciding where it belongs.
