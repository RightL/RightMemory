...
- If yes, send a retrieval request to the curator subagent and wait for its reply. Let the curator choose which `MEMORY*.md` files are relevant. Use the returned addressable lines as context; quote them verbatim when relying on them — do not paraphrase heading ids, node ids, descriptions, or edges.
