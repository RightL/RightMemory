...
- When retrieval is needed, send a request to the curator subagent and **wait at least 3 minutes for its reply before acting** on that memory. The curator skips items already returned in this session; ask explicitly if you need something again. Use the returned addressable lines as context; quote them verbatim when relying on them — do not paraphrase heading ids, node ids, descriptions, or edges. If current work shows retrieved memory is stale, wrong, too broad, or misleading, preserve the returned address and the correction needed for the next update brief.
...
- For updates, send a brief to the same curator subagent and **proceed without waiting** for its reply (fire-and-forget).
- The first update in a curator session should include fuller surrounding context: meaning, relevance, and relationship to existing memory.
- Later updates in the same curator session may be shorter when earlier dispatch context is enough. Include fresh context when the meaning changed or depends on details not yet sent.
- For corrections to retrieved memory, describe the stale or wrong memory well enough for the curator to find it, and say whether it should be revised, narrowed, or deleted.
