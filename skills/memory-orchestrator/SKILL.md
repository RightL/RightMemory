...
- The dispatch prompt sent to the curator subagent should (a) point it at `{{SKILLS_ROOT}}/memory-curator/SKILL.md` and tell it to follow that skill, (b) for retrieval, describe the memory needed based on the user's intent instead of blindly forwarding the user's message verbatim, and (c) include a concrete change brief when the task is an update.
