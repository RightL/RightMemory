...
- For updates, send a brief to the same curator subagent and **proceed without waiting** for its reply (fire-and-forget). Include enough surrounding context for the curator to understand meaning, relevance, and relationship to existing memory. For corrections to retrieved memory, name the returned heading/node/line and say whether the old memory should be revised, narrowed, or deleted.
