...
- Edit in the worktree. Preserve existing ids across rename and move. Titles and ids are optional; generate a globally unique id when graph relationships, Focus, or a typed backing need one. Keep each direction's tree position, optional title and own Markdown body, and useful graph relationships. Use a body fence for lists or headings within that body, and preserve the complete content and order of existing leaf items. Execution detail and completed history belong outside the map.
