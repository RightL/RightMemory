...
- If retrieved Memory or Agent Corrections are stale, wrong, misleading, or overbroad, do not follow them. Submit the problem and current evidence immediately without waiting for approval. Treat stale Pursuit as read-only context too; task activity does not authorize correcting the map.
