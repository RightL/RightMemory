...
- Do not submit correction evidence during an active discussion. Submit a correction with a clear outcome before a response after which the user is unlikely to send another substantive message about that work. Include enough context to show the relevant request, what the agent proposed, did, or omitted, how the user redirected it, and the outcome. If a task-state candidate is due at that boundary, include the correction in it instead of submitting separately.
