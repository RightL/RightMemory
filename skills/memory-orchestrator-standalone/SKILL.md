...
- For updates, call `rightmemory update submit --session <stable-session-id> "<concrete candidate brief>"` and proceed without waiting or pulling for the update result. The standalone runtime accumulates submitted candidates, waits 1 hour from the latest submit, and sends the pending candidates to the update role as one batch. Include enough surrounding context for the updater to understand meaning, relevance, uncertainty, and relationship to existing memory. For corrections to retrieved memory, name the returned heading/node/line and say whether the old memory should be revised, narrowed, or deleted.
