...
- For updates, call `rightmemory curator submit --session <stable-session-id> "[UPDATE] <concrete change brief>"` and proceed without waiting or pulling for the update result. If another update is already running for the same session, the standalone runtime queues the new update. Include enough context for the curator to place the memory well: what changed, why it matters, and any known scope or related memory. Do not invent unknown structure, and do not pre-write the diff — let the standalone curator choose structure.
- When the user asks for a memory-update result or status, call `rightmemory curator pull --session <stable-session-id>`; the output includes current and queued update information.
