...
- When retrieval is needed, wait for the curator command before acting on that memory. The curator skips items already returned in this session; ask explicitly if you need something again. Use returned addressable lines as context; quote them verbatim when relying on them — do not paraphrase heading ids, node ids, descriptions, or edges.
...
- For updates, start `rightmemory curator --session <stable-session-id> "<concrete change brief>"` and proceed without waiting for its reply (fire-and-forget): information type, affected heading / node / section if known, what changed, relations, and reason. Do not pre-write the diff — let the standalone curator choose structure.
