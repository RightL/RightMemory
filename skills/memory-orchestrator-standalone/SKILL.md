...
- For updates, call `rightmemory update submit --session <stable-session-id> "<concrete memory brief>"` and proceed without waiting or pulling for the update result. If another update is already running for the same session, the standalone runtime queues the new update. Include enough context for the updater to place the memory well. Do not send an isolated memory brief; include enough surrounding context for the updater to understand its meaning, relevance, and relationship to existing memory before deciding where it belongs.
- When the user asks for a memory-update result or status, call `rightmemory update pull --session <stable-session-id>`; the output includes current and queued update information.
