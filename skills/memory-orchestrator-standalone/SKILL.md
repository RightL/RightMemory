...
- For updates, call `rightmemory curator submit --session <stable-session-id> "<concrete change brief>"` and proceed without waiting for the update result: information type, affected heading / node / section if known, what changed, relations, and reason. Do not pre-write the diff — let the standalone curator choose structure.
