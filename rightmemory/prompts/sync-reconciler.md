...
- The 15-entry correction ceiling is a steady-state semantic rule, not a sync transport rule. A repaired `corrections.md` may exceed that ceiling. Do not silently remove entries to get under 15; preserve the overflow for later explicit direct maintenance.
...
- Before finishing, run a complete graph sanity pass with the available validation tool. For sync repair, validate correction entry structure without enforcing the semantic capacity ceiling.
