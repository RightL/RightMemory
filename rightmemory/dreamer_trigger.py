...
class DreamerTriggerStore:
    ...
    def increment_once(self, operation_id: str, points: float) -> DreamerTriggerState:
    ...
    def claim_operation(self, threshold: float) -> str | None:
        ...
        with self._locked():
            ...
            if state.points < amount:
                return None
            operation_id = f"dreamer-watch-{uuid.uuid4().hex}"
            self._write_locked(
                replace(
                    state,
    ...
    def complete_operation(self, operation_id: str, threshold: float) -> bool:
        clean_operation_id = _operation_id(operation_id)
        ...
        with self._locked():
            ...
            if state.points < amount:
                raise RuntimeError("dreamer trigger points changed below the claimed threshold")
            now = _now()
            self._write_locked(
                replace(
                    state,
