...
Paste a PNG or JPEG into the composer to attach it to the next message. A large text paste becomes a staged text attachment instead of filling the editor with thousands of lines. RightMemory keeps these inputs in root-local runtime state. For an SSH conversation, the backend copies them to the conversation host before starting the turn, so Codex receives paths it can access there. Staged inputs are conversation workspace files, not Pursuit notes or graph resources.
