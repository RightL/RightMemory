...
Each write carries the revision it was based on. When sync, an agent, or another writer advances the root, a pending batch automatically includes committed changes to files outside its saved actions and undo/redo history. Background queue records therefore do not interrupt editing. The combined graph must remain valid; overlapping file changes retain both versions and report a conflict. A second browser session cannot write while another session has pending edits. Failed title and Note drafts stay available for review and retry in the current page; preserve needed draft text before closing or reloading it.

After a restart, the editor restores saved actions and applies the same check for unrelated committed changes. It preserves each action's exact file contents and undo/redo history without replaying operations. Recovery records and session action history live in the root's `.runtime/pursuit-editor/` directory, outside Git; preserve this directory when resolving a reported recovery conflict. Canonical Pursuit remains Markdown and Git.
