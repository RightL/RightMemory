...
Normal submitted work batches for token efficiency and memory quality, but failed submitted work uses a separate recovery lane. Once a queue has already failed, preserving backlog correctness matters more than waiting for a full batch, so retryable failures cool down and then run without the normal candidate-count threshold. Repeated failure stops in manual recovery instead of silently looping forever. `submit` saves new candidate work; `retry` owns recovery of blocked queues, which keeps severe update failures visible to the agent and gives the user one explicit global recovery action.
