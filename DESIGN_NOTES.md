...
Titles and body previews are display text. Durable addresses come from explicit ids; anonymous editing handles bind to the complete document snapshot and become invalid when it changes. Retrieval keeps its existing selection format and supplies missing ids in a reading copy built from the document index. These ids remain bound to the indexed snapshot in the retrieval session; selections resolve to original blocks and delivery versions exclude generated labels. A selected heading covers its descendants, while a selected leaf includes its entire Markdown container and ancestor-owned context.
