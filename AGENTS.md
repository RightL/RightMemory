...
- Dirty main memory files block automatic semantic writes, but runtime gives `sync-reconciler` one bounded chance to repair local dirty state before failing the automatic write. Temporary commits may touch `MEMORY.md`, `MEMORY_*.md`, and `dream_logs/*.md`, and each temporary commit must keep `MEMORY.md` as a regular file. Failed isolated work is discarded and retried from the original source state.
