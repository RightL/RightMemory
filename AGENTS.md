...
- Use `rightmemory watch start|status|stop|restart` to manage transcript review, update review, dreamer, insight, pruner, CLI-agent cleanup, and sync watchers. Use the corresponding direct `watch` command when debugging a lower-level loop.
- Use `rightmemory agent-cli cleanup --once` to run one bounded registered-Codex-thread cleanup pass.
...
- In CLI-agent mode, `retrieve` is the only role that persists a provider mapping across independent commands. Other role commands and automatic cycles are one-shot; explicit chat may retain only process-local continuity.
- CLI-agent ownership records live under `.runtime/agent_cli_threads/`. Registered Codex threads expire after 24 hours without successful activity and are deleted only through Codex App Server; never discover or delete unregistered history and never mutate Codex files or SQLite directly.
...
- Automatic unified-update, dreamer, insight, and pruner turns that operate on the main state root run in isolated `.runtime/worktrees/` checkouts on `rightmemory-isolated-<role>-<uuid>` branches. Runtime validates and lands successful role-owned commits, then promotes temporary session/provider state. CLI-agent isolated turns use a fresh one-shot provider session; their ownership record is promoted on success and preserved alone after later isolation failure so review exclusion and cleanup remain safe. Transcript review is read-only extraction and queues any resulting candidate instead of entering the semantic-write path.
