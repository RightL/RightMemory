...
The conversation pane supports per-message model and reasoning choices, phase-aware commentary that folds after the final answer, sanitized Markdown and TeX, and live subagent activity derived from App Server thread items. Pasted PNG or JPEG images and large blocks of text are staged below the active root's `.runtime` directory. For an SSH conversation, RightMemory copies that staged input to the conversation host before the turn starts, so Codex receives a host-local image or text path rather than a path that exists only in the browser's machine. This attachment-only transfer requires `python3` on the remote command path. If it is unavailable, messages without attachments still work; an attached send fails before its turn starts and keeps the local staged input available to retry or remove. Cleanup of a remote staged file is best effort and can leave the file under `~/.cache/rightmemory/attachments/` when the host or `python3` is unavailable.
