...
The conversation pane supports per-message model and reasoning choices, an immediate Working surface with provider-produced reasoning summaries and tool activity, phase-aware commentary that folds after the final answer, sanitized Markdown and TeX, and live subagent activity derived from App Server thread items. Newly received provider raw reasoning content is discarded rather than displayed or stored. While a turn is active, the primary Send control becomes Stop and interrupts that App Server turn. Paste a PNG or JPEG, paste a large block of text, or use the paperclip to stage general files below the active root's `.runtime` directory. A message can contain up to eight staged inputs, including up to four images; each image or general file may be up to 20 MiB, while a managed text paste may be up to 5 MiB. PNG and JPEG inputs use App Server's image input. Other files are exposed to Codex through a managed absolute path, so their contents can be inspected only when the conversation's sandbox and available tools support the format; staging a file does not guarantee native understanding of every binary format. For an SSH conversation, RightMemory copies the staged input to the conversation host before the turn starts. This attachment-only transfer requires `python3` on the remote command path. If it is unavailable, messages without attachments still work; an attached send fails before its turn starts and keeps the local staged input available to retry or remove. Cleanup of a remote staged file is best effort and can leave the file under `~/.cache/rightmemory/attachments/` when the host or `python3` is unavailable.
...
The conversation pane also has a fixed Manager entry for persistent, root-local Codex conversations. Manager runs on the Web Studio controller with its working directory fixed to the active RightMemory root, has no Pursuit attachment, and does not contribute to Pursuit activity indicators. Opening it can stage the current map selection as a removable reference for the next message. Manager can perform explicitly requested Memory, Pursuit, host, and project maintenance through the same validated files and Web Studio business APIs; it does not introduce a separate database, remote tool framework, or routine proposal-and-approval ceremony. `rightmemory manager` is the thin authenticated loopback client for those registered map and execution-configuration operations.
